...
from SaitamaRobot import (EVENT_LOGS, LOGGER, OWNER_ID, DEV_USERS, DRAGONS, DEMONS, TIGERS, WOLVES,
                          dispatcher)
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if user_id == OWNER_ID:
        message.reply_text(" The ICE KING cannot be fed banned!")
        return

    if int(user_id) in DRAGONS:
        message.reply_text("Imperial Soldiers cannot be fed banned!")
        return

    if int(user_id) in DEMONS:
        message.reply_text("Royal Army Soldiers cannot be fed banned!")
        return

    if int(user_id) in DEV_USERS:
        message.reply_text("Night Raiders can be fed banned!")
        return

    if int(user_id) in TIGERS:
        message.reply_text("Military Soldiers cannot be fed banned!")
        return

    if int(user_id) in WOLVES:
        message.reply_text("Loyal Servants cannot be fed banned!")
        return
